...
4. If the user explicitly asks for `/forge:review`, route directly to the
   internal review payload (`references/temper/SKILL.md`) after preflight
   instead of running the full discovery-to-implementation workflow. Treat
   phrases such as `against base`, `against main`, `against develop`, or
   `against <branch>` as a requested comparison base for the internal review
   run. For example, `/forge:review against <branch>` carries `<branch>` as
   that base.
...
- whether `/forge:review` was explicitly requested, and any base-branch
