...
Applies to the changed SQL queries and indexes in `temper` Phase 1 (the diff, not the whole schema). Engine-agnostic (MySQL, PostgreSQL, SQL Server, Oracle). The Phase-0 project context capsule and standing project instructions always override these generic rules.
