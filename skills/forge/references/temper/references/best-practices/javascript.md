...
Applies to the changed JavaScript code in `temper` Phase 1 (the diff, not the whole repo). For TypeScript *type* idioms see `typescript.md`; for React-specific guidance see `react.md`; for SOLID/composition/layering see `general-oop.md`; for UI accessibility & i18n see `frontend-a11y-i18n.md`; for stylesheet quality see `css.md`. The pattern catalogues below are distilled from patterns.dev. The Phase-0 project context capsule and standing project instructions always override these generic rules.
