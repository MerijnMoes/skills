...
// When a newer upstream payload ships, append an UPDATE_AVAILABLE directive so
// forge maintainers can record source drift. Everything here is best-effort and
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer upstream design-studio payload is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Do not install or update external packages from inside forge. ` +
    `Record this as source drift for the forge maintainers and continue the current task.`
  );
}
...
async function cli() {
  ...
  if (!ctx.hasProduct) {
    ...
    const parts = [
      ...
      'Stop the current task, load references/design-studio/reference/init.md, and follow its ' +
  ...
  const next = register
    ? `NEXT STEP: This project's register is \`${register}\`. You MUST now read \`references/design-studio/reference/${register}.md\` before producing any design output.`
    : `NEXT STEP: You MUST now read the matching register reference (\`references/design-studio/reference/brand.md\` or \`references/design-studio/reference/product.md\`) before producing any design output. Pick based on PRODUCT.md above.`;
  parts.push(next);
  if (updateDirective) parts.push(updateDirective);
  process.stdout.write(parts.join('\n\n---\n\n') + '\n');
}
