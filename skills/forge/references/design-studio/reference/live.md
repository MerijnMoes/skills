...
```
LOOP:
  node $FORGE_SKILL_DIR/references/design-studio/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-status.mjs
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-resume.mjs --id SESSION_ID
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
Any other `event.action` (`bolder`, `quieter`, `distill`, `polish`, `typeset`, `colorize`, `layout`, `adapt`, `animate`, `delight`, `overdrive`): Read `references/design-studio/reference/<action>.md` before planning. Each sub-command encodes a specific discipline; skipping its reference produces generic output. Those files may require specific params; layer them on top of the §7 budget, not instead of it.
...
**Signature params per action.** For named sub-commands, read that action’s `references/design-studio/reference/<action>.md` for one or two **MUST** params (e.g. `layout` → `density`). Those are non-negotiable when the design can express them. **Freeform has no file-level MUST**; the **Freeform (`impeccable`) bias** in this section is the stand-in. If the user’s action is both stylized and sub-command (e.g. `colorize`), the sub-command’s MUST list takes precedence for its axes; still respect the **Hard cap** and add no redundant duplicate knobs.
...
```bash
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node $FORGE_SKILL_DIR/references/design-studio/scripts/live-server.mjs stop
```
...
```bash
node $FORGE_SKILL_DIR/references/design-studio/scripts/detect-csp.mjs
```
