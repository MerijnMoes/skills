...
5. **Build a risk map.** Follow `references/risk-mapping.md` to classify the change and write down the top risks before you start polishing. Capture the change archetypes in play (e.g. UI, API, auth, persistence, schema/migration, async/job, integration, config/feature-flag, perf-sensitive), the core invariants, the side effects, the trust boundaries, and the 2-5 highest-value failure modes to probe later. This risk map drives Phases 4, 6, and 7.
6. **Read conventions:** root and nearest `CLAUDE.md` / `AGENTS.md`, plus linter/formatter config, so later phases respect house style.
...
Gate: you have a clear diff, a language list, a risk map, a pinned intent (or an explicit note that none exists), and a green (or explicitly-acknowledged) starting state.
...
Use the Phase-0 risk map to decide which conditional lanes deserve real attention. `/finalize` should not pretend every diff has the same risk profile.
...
Gate: no blocking review, security, dependency, static-intelligence, bug-hunting, consistency, spec-conformance, or structural findings remain; no secrets in the diff. Blocking findings are verified (reproducible trigger), not speculative.
...
3. **Behavioral check:** follow **`references/verify.md`** to actually run the app/feature and observe real behavior, not just green tests. Type-checks and tests prove code correctness, not feature correctness. Build the verification plan from the Phase-0 risk map: always exercise the golden path, at least one meaningful negative/error path, and at least one risk-specific probe for each top failure mode that matters (retry/double-submit, persistence round-trip, old/new data shape, auth boundary, stale cache, etc.). Use the required-probe guidance in `verify.md` so every archetype in the risk map gets at least one meaningful probe. For UI/web changes, if Playwright is already configured in the repo or otherwise already available in the environment, prefer a focused reproducible browser flow for the highest-value journey and one meaningful negative/regression path rather than an ad hoc click-through.
...
- Follow `references/validation-gate.md` exactly — it is a 12-section checklist (including business-risk lanes for data integrity, idempotency/concurrency, and financial correctness) plus a required final risk pass. Reason about each item against the diff, the Phase-0 risk map, and the evidence gathered in Phase 6; flag issues rather than assuming correctness. Use the checklist as a floor; bring forward any residual bug hypotheses from `references/bug-hunting.md` that the checklist doesn't name explicitly.
